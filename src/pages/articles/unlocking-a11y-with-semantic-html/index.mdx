...
<div className="not-prose mb-4 py-4 grid grid-cols-1 gap-8 lg:grid-cols-2">
  <div
    ...
    className="inline-block text-center rounded-sm bg-gray-200 p-2 text-sm text-black transition-colors hover:bg-gray-300"
    onClick={(e) => {
  ...
  <button
    data-message="Yay, you clicked the button!"
    className="inline-block rounded-sm text-center bg-gray-200 p-2 text-sm text-black transition-colors hover:bg-gray-300"
    onClick={(e) => {
      alert(e.currentTarget?.getAttribute('data-message'));
    }}
  >
